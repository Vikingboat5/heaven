# UI 与动效规范 (v1.3, 2026-09-12 拍板)

> **地位**: 所有前端页面/组件的实现与验收依据之一（与 seed-item-system-spec §11 操作预期表并列）。
> 新功能拓展时必须先读本规范；规范没覆盖的样式决策，先在本文档补条目再动手（对齐 CLAUDE.md C2）。
> 落地位置：tokens 在 `frontend/src/style.css`，本文件是"为什么这么定 + 怎么用"。
>
> **v1.1 变更**（2026-08-31，负责人提供《旅行青蛙》《农场》参考图拍板"布局+材质全做"）：
> - 新增 §3.2 主页环形贴边布局（替代底部操作坞）
> - 新增 §5 材质体系：木牌 / 纸张 / 玻璃的分工
> - §0 原则 2 修订：温暖绘本感 = 黄昏夜景 + **手绘实物材质（木/纸）**
> - Backlog：宠物在家生活行为（学旅行青蛙：吃东西/睡觉而非站桩，需 petgen 新动作帧，另排期）
>
> **v1.2 变更**（2026-09-06，负责人拍板"素材全面 AI 化 + 动画做减法"）：
> - 木牌/便签/图标全部替换为 **AI 手绘素材**（`backend/static/ui/`，`scripts/gen_ui_assets.py` 生成，与场景同画风；木牌底图禁止带孔洞——孔洞会让文字透底）
> - 宠物 idle 改为**基础循环**（摇尾巴+中间眨一次眼，12 帧 @140ms），不做复杂编排
> - 素材 bbox 铁律：alpha 羽化会让包围盒失效，裁剪必须按**最大连通域 + alpha>24**（`scripts/fix_asset_bboxes.py`）
>
> **v1.3 变更**（2026-09-12，负责人拍板"游戏化交互"）：
> - **底部 tab 导航**：小窝 / 日记 / 背包（收藏）三个同级 tab，AI 图标+激活弹跳（替代草地上的日记本/篮子入口；「打包出门」背包留在场景里作为主动作）
> - **明信片墙独立成页**：点窝边照片进入，明信片**挂在夜空麻绳上**（小木夹+暖光晕+轻摇；v1.3.1 订正：软木板与梦幻夜景调性不符，改为挂绳方案）
> - **动效补全**：按钮按压改 spring 回弹曲线；tab 切换图标 pop；页面转场保持 fade+上移

## 0. 设计原则

1. **宠物是主角**（布局红线，2026-08-23 拍板）：任何 UI 元素不得遮挡宠物形象；场景中心区域（宠物±一圈）禁止放置任何 UI；弹层优先底部抽屉（bottom sheet），不做全屏居中卡
2. **温暖绘本感**：黄昏夜景 + 手绘实物材质（木牌/纸张）+ 圆角 + 柔和光晕；拒绝锐利直角、高对比硬边、纯黑纯白；**功能物 = 场景里的实物**（按钮是木牌，信是钉在树上的便签，可交互的场景物带光晕）
3. **动静有度**：动效服务于"它活着"的感觉（呼吸、漂浮、错峰入场），不做炫技式动画；所有动效可降级（`prefers-reduced-motion`）
4. **一致性优先于个性**：新组件必须复用 tokens 与既有模式；禁止引入"只此一处"的颜色/字号/时长

## 1. 色彩

tokens（`style.css` `:root`）：

| token | 值 | 用途 |
|-------|-----|------|
| `--color-night` | #150c2e | 全局底色 |
| `--color-primary` | #f2a56e | 主行动（渐变起点） |
| `--color-accent` | #e77fa2 | 点缀（少用） |
| `--color-gold` | #f7c98a | 标题/数字/强调文字 |
| `--color-text` | #fdf6ec | 主文字 |
| `--color-text-dim` / `--color-text-faint` | 60% / 40% 透明度 | 次要/弱化文字 |
| `--color-glass` / `--color-glass-border` | 半透明深紫 | 玻璃卡片底/边 |

**品级色**（图鉴/背包/信件共用，禁止另造）：
- 常见 common：中性灰边 `rgba(255,255,255,0.12)`
- 稀有 rare：蓝 `#78aaff`（描边 55-60% 透明度 + 微光晕）
- 传说 epic：金 `#f7c964`（描边 65-80% + **旋转流金边框** `.epic-glow`）

规则：渐变按钮（蛋期 CTA 等）统一 `linear-gradient(135deg, #ffd9a0, #f2b06e)`；错误/提示红用 `#ff7a7a` / `#ffb3b3`；不新增品牌色。

**材质色（v1.1 新增）**：

| token | 值 | 用途 |
|-------|-----|------|
| `--gradient-wood` | #b8894f→#96683a→#7d5530 斜向 | 木牌底 |
| `--color-wood-edge` | #5d3d22 | 木牌描边 |
| `--color-paper` / `--color-paper-edge` | #f7ecd4 / #d9c4a3 | 纸张底/边 |
| `--color-paper-text` | #4a3320 | 纸上文字（深棕） |
| 纸上品级文字 | 常见 #8a7a5f / 稀有 #4a6fa5 / 传说 #b8862e | 纸面上品级色压暗一档 |

### 3.2 主页环形贴边布局（v1.1，替代底部操作坞；参考《旅行青蛙》《农场》）

宠物出现后，**中心留给场景，UI 环形贴边、各司其职**：

| 位置 | 元素 | 材质 |
|------|------|------|
| 顶左 | 宠物信息牌（名字/种族·等级/性格标签/天赋技能一行） | 木牌 `.pet-plaque` |
| 左下 | 主动作「打包行李」 | **场景实物直放**（背包，无底板） |
| 右下 | 次导航「收藏」 | **场景实物直放**（篮子，无底板） |
| 底部居中 | 旅行中状态横幅（目的地/ETA/行囊只读，一条细纸带） | 纸张 `.away-banner` |
| 树上（右侧） | 归来便签信（图钉+微浮动，不自动弹） | 纸张 `.pinned-note` |
| 顶部居中 | 品牌标题 | **宠物出现后隐去**（蛋期才显示） |

> 主页入口不做底板（2026-09-06 拍板）：场景里没有"挂板子"的依托，贴块牌子就是 UI 贴纸；入口应是场景里的实物（背包/篮子），文字用奶白+深色投影直接落在草地上。木牌底板只用于内容页 CTA（如打包页「送它出门」）。

蛋期/孵化期（loading/egg/ready）仍用底部紧凑玻璃坞 `.panel`（过渡阶段，不占红线位）。

## 2. 字体与排版

- **标题/宠物名/大数字**：得意黑 Smiley Sans（`--font-display`，npm 包 `cn-fontsource-smiley-sans-oblique-regular`，按需分包 woff2，`font-display: swap`）
- **正文/日记**：圆体栈（`--font-body`: Yuanti SC → YouYuan → PingFang SC → Microsoft YaHei → system-ui）
- 字号阶梯（只能用这几档）：

| token | 值 | 用途 |
|-------|-----|------|
| `--fs-micro` | 10px | 角标/品级微标签 |
| `--fs-xs` | 11px | 角标/辅助说明 |
| `--fs-sm` | 12px | 次要信息/提示 |
| `--fs-md` | 13px | 表格/次要按钮 |
| `--fs-lg` | 14px | 正文/日记/**所有按钮文字（含入口木牌、页级 CTA）** |
| `--fs-xl` | 17-18px | 卡片标题 |
| `--fs-xxl` | 22px | 页面大标题/大数字 |
| `--fs-display` | 28px | 品牌标题（仅此一处） |

- **同层级同字号铁律（v1.2 拍板）**：同一页面同一层级的元素（如主页两个入口木牌）必须同字号；主从层级差异通过**尺寸/位置**表达，不通过字号
- 标题字距：`letter-spacing: 3-6px`（大标题）/ `1-2px`（卡片标题）；正文不加字距
- 数字（经验/数量/倒计时）一律 `font-variant-numeric: tabular-nums`

## 3. 布局

- 外层 shell：max-width 480px 居中（手机优先），顶部 sticky 导航（家园/收藏/用户）
- 主页：场景层（SVG 插画）→ 宠物层 → UI 层（操作坞贴底，`bottom: 14px`；占屏比红线：宠物在家 ≤15%，旅行中（含状态/行囊信息）≤20%，以 1221px 高视口实测为准）
- 内容页（打包/收藏）：统一页面骨架 `.page`（深色渐变底 + 双星闪烁 + 顶部返回+标题区 + 内容区）

### 3.1 主页场景层构成（2026-08-29 场景升级拍板）

画家算法自下而上，**新元素必须插入正确层级**，且全部走 `prefers-reduced-motion` 降级：

| 层 | 元素 | 动效 |
|----|------|------|
| 天空 | 8 档黄昏渐变 / 38 星（白·暖黄·淡蓝三色温）+ 5 十字星芒 / 银河带（18 碎星+柔光带）/ 月牙+银辉 / 流星 ×2（12s、17s 错峰） | twinkle / meteor-fly |
| 远景 | 落日（blur24 大柔光）/ 地平线暖雾带 / 4 层远山 + 2 条山雾带 / 山谷灯火 ×6（远处村庄）/ 山脊剪影树丛 | 灯火 twinkle |
| 中景 | 孤独的树 + 木漏光斑 ×4 / 挂灯 ×2（灯核+大光晕, 摆动）/ 飘落花瓣 ×5 / 小水洼（映月+涟漪 ×2） | lantern-sway / petal-fall / ripple-spread |
| 巢区 | 巢 + 呼吸光晕 + 地面反光 + 尘埃光粒 ×4 + 宠物 | halo-breathe / mote-rise |
| 氛围 | 萤火虫 ×12（双层光晕：blur6 晕 + 实心核）/ 前景草叶（微风摆动） | firefly / grass-sway |
| 收尾 | 暗角 vignette（聚焦巢区）+ 纸感噪点 | — |

约束：氛围元素（流星/花瓣/萤火虫）必须"克制"——长周期、低占空比；场景永远服务于宠物，不在宠物身上叠加任何场景元素。

### 3.3 AI 场景分层体系（2026-09-06 拍板，方向：素材全面转 AI 生成）

主页场景从纯 SVG 过渡为 **AI 生成图层 + CSS 动效叠加**：

| 层 | 内容 | 来源 | 说明 |
|----|------|------|------|
| 0 天空 | 渐变+星星+月亮 | **CSS**（不用 AI） | 纯色域 CSS 更准，还能按真实时间换色温 |
| 1 远景 | 远山/树/灯火剪影 | AI 生图 + 白底抠图 | 深蓝紫剪影对白色背景对比强，抠图稳 |
| 2 地面 | 草地+巢 | AI 生图 + 白底抠图 | **必须带一片平坦空地 = 宠物锚点** |
| 3 前景 | 草丛边角 | AI 生图 + 白底抠图 | 可选 |
| 4 宠物 | PetSprite | 现有 | 锚点 (anchorX%, anchorY%) 按场景包配置 |
| 5 动效 | 萤火虫/流星/花瓣 | 现有 CSS | 叠加在所有层之上 |

铁律：
1. **宠物锚点必须经合成目检验收**：生成后用脚本把宠物帧合成到锚点，人工（或视觉模型）确认位置自然再上线
2. **图层即场景包**：远景×地面可自由组合（屋内/屋外/沙滩……），新增场景 = 新增图层包，不改代码
3. 抠图复用 petgen 管线的白底三件套工艺；所有图层 PNG 带 alpha
4. AI 素材生成脚本统一走 `scripts/gen_scene_layers.py`，原图+参数落盘可离线重切（同 petgen）
- 弹层：底部抽屉（贴底、顶部圆角 24px、max-height 62vh、遮罩 45% 透明度）；详情小卡可居中（图鉴物品详情等"对单物聚焦"场景例外）

## 4. 动效规范

tokens：

| token | 值 | 用途 |
|-------|-----|------|
| `--motion-fast` | 150ms | 按压/hover |
| `--motion-med` | 250ms | 入场/徽章 |
| `--motion-slow` | 400ms | 抽屉/弹层 |
| `--ease-out` | cubic-bezier(0.22, 1, 0.36, 1) | 默认出场 |
| `--ease-spring` | cubic-bezier(0.34, 1.56, 0.64, 1) | 回弹感（徽章/信封/入槽） |

**标准模式**（复用，别新造）：
1. **按压反馈**：所有可点元素 `:active { transform: scale(0.96) }`，过渡 `--motion-fast`
2. **错峰入场**：列表/网格容器加 `.stagger`，子元素 fade+上移 12px，`--motion-med`，每个延迟 40ms（CSS nth-child 到 12 个）
3. **页面转场**：router-view fade + 上移 8px，180ms `--ease-out`
4. **徽章脉冲**：NEW! 角标 `.badge-pulse`，1.6s 缩放 1→1.12→1 + 光晕
5. **信封**：`envelope-bob` 1.6s 上下浮动 8px（无限）
6. **信件抽屉**：`sheet-in` 从底部上移 60px 淡入，`--motion-slow`
7. **传说流金**：`.epic-glow` 用 `@property --angle` + conic-gradient 旋转 6s
8. **降级**：`@media (prefers-reduced-motion: reduce)` 下所有动画关闭（全局兜底已含）

禁止：无限大幅位移动画（除信封/萤火虫等氛围元素）；超过 500ms 的交互动效；`transition: all`（写明确属性）。

## 5. 组件模式

**材质分工（v1.1 铁律）**：木牌 = 操作按钮/信息牌；纸张 = 信件/便签/状态横幅；玻璃 = 内容页卡片 + 蛋期操作坞。同一场景不混用同语义材质。

- **木牌按钮 `.wood-btn`**（全局类）：`--gradient-wood` 底 + `--color-wood-edge` 2px 描边 + 内高光/内阴影 + 得意黑 + 字距 4px + 奶白文字带深色投影；用 `rotate` 属性（非 transform，避免与按压 scale 冲突）给 ±1.5° 手工感
- **纸张 `.paper` 系**：`--color-paper` 渐变底 + `--color-paper-edge` 1px 边 + 深棕文字；图钉（便签）用 radial-gradient 红珠
- **玻璃卡**：`--color-glass` 底 + blur(14-18px) + `--color-glass-border` 边 + 圆角 16-20px
- **次按钮 action-btn**（内容页）：玻璃底 + 细边，圆角 12px
- **物品格**：图（44px 圆角 8px）+ 名 + 品级色描边；未获得=剪影 `grayscale(1) brightness(0.35)` + 名称 "???"
- **空态**：emoji/插画 + 一句标题 + 一句引导（禁纯文字报错感）
- **加载态**：爪印/圆点 loader，禁裸 "加载中…" 大字（短期可先用文字，迭代时换插画）

## 6. 验收 checklist（每个新页面/组件过一遍）

- [ ] 只用了规范内的色/字号/时长（无硬编码新值）
- [ ] 可点元素有按压反馈；列表有错峰入场
- [ ] 不遮挡宠物（涉及主页时，几何验证：sprite 矩形与 UI 矩形无交集）
- [ ] 品级色一致；数字 tabular-nums
- [ ] `prefers-reduced-motion` 下不闪烁不位移
- [ ] console 零错误
